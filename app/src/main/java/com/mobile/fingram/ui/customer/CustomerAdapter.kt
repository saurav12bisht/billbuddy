...
import com.mobile.fingram.databinding.ItemCustomerBinding
...
class CustomerAdapter(
    ...
    class VH(
        ...
        fun bind(item: CustomerWithBalance) {
            ...
            b.tvPhone.text = com.mobile.fingram.ui.common.util.DateFormatter.formatPhone(item.phone)
            b.tvBalance.text = CurrencyFormatter.formatPaiseToRupee(item.balance)
            b.tvBalance.setTextColor(
                b.root.context.getColor(
                    when {
                        item.balance > 0 -> com.mobile.fingram.R.color.accent_red
                        item.balance < 0 -> com.mobile.fingram.R.color.paid_success
                        else -> com.mobile.fingram.R.color.text_secondary
                    }
                )
            )
